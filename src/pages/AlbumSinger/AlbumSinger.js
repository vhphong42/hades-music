...
function AlbumSinger() {
    ...
    useEffect(() => {
        if (dataFullSongs.length === 0 && !slugBannerAlBumHot) {
            ...
            const fetch = async () => {
                try {
                    await axios.put(`${process.env.REACT_APP_URL_ZING}/artists/update-play-count/${nickname}`);
    ...
    useEffect(() => {
        if (isBannerAlbumHot) {
            ...
            const fetchBannerAlbumHot = async () => {
                await axios.put(`${process.env.REACT_APP_URL_ZING}/albums/update-play-count/${nickname}`);
                const result = await axios.get(`${process.env.REACT_APP_URL_ZING}/albums/by-music/${nickname}`);
                console.log("result", result.data)
                

                const newDataFillter = result.data
                    ...
                    {isLoadingPage || dataFullSongs.length === 0 ? (
                        ...
                        <h2 className={cx('title_header')}>
                            ...
                            <span className={cx('title_extra')}>
                                <Link to={`/${dataInAlbum.name_singer}`}>
