        ...
        <div className={cx('item')}>
            ...
            >
                {isPodcast ? (
                    ...
                    >
                        <Images
                            src={ item.src||  item?.image_artist || item?.image_album }
                            style={{
                                width: '100px',
                                height: '100px',
                            }}
                    ...
                    <Link
                        to={`/album/${
                            item?.slug_name_artist || item?.slug_album   
                        }`}
                        state={{
                            src: item?.image_artist || item?.image_album  ,
                            title: item?.name_artist || item?.name_album,
                            slug_banner_singer_popular: item?.slug_name_artist,
                            slug_banner_album_hot: item?.slug_album,
                            isBannerAlbumHot: item?.slug_album ? true : false,
                        }}
                    >
                        <figure className={cx('item_img')}>
                            <Images src={item?.image_artist || item?.image_album || item?.src} />
                        </figure>
                        <div
                ...
                <div>
                    ...
                    >
                        <h3 className={cx('item_title')}>
                             ...
                             {item.name_artist  ?  "Những bài hát hay nhất của" +item.name_artist : item.name_album   || item.slug_banner_singer_popular} 
                        </h3>
                    </Link>
                    <span className={cx('item_extra_title')} key={index}>
                        {Array.isArray(item.name_data) ? (
                            item.name_data.map((singer, index) => {
                                ...
                                return (
                                    <Link to={`/${singer?.slug_name_artist}`} key={index}>
                                        {singer.name_singer +
                                            (index === dataLength ? '' : ', ')}
                                    </Link>
                                );
                            ...
                            <Link to={`/${item?.slug_name_artist ||item?.artist?.name_artist  }`}>
                                {item.name_artist || item?.artist?.name_artist  || item?.slug_banner_singer_popular}
                            </Link>
                        )}
                    </span>
                </div>
