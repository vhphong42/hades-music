...
import { ListQueue, Mic, Multi, Mv, Volumn, VolumnOff } from '../../../components/Icons';
...
function ControlsRight({ isMobile, isControlModal }) {
    ...
    const CONTROL_BTNS_RIGHT = [
        {
            data: [
                // {
                //     icon: Mic,
                //     disable: !songCurrent?.content, // Kiểm tra xem có lời bài hát hay không
                //     type: 'lyrics',
                //     extraTitle: 'Lời bài hát',
                //     content: songCurrent?.content ? songCurrent?.content.split('.').map((line, index) => (
                //         <p key={index}>{line.trim()}</p>
                //     )) : null, // Nếu có content, split ra từng dòng và hiển thị
                // },
    ...
    const handle = (action) => {
        switch (action) {
            ...
            case 'lyrics':
                dispatch(statusSlice.actions.isLyricsPlayerChange(true));
